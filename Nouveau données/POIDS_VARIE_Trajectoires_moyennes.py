...
def MH_DOC (c1,c2,c3, Xi,Xf):           
    ...
    opti.minimize(  taux*( dot(c1 *u1,u1) +  dot(c2 *u2,u2 ) + dot(c3 *u3 ,u3 )  ) )   
...
def MH_PDFO (C):
    ...
    for j in range (40):
        ...
        X1_1, X2_1, X3_1  = MH_DOC (c1,c2,c3, Xi,Xf)
...
res = pdfo( MH_PDFO, [1/3, 0, 0, 0, 0, 0,1/3, 0,0,0,0,0,1/3, 0,0,0,0,0], constraints=Lin_const, options=options)
...
for i in range (40):   
    ...
    X,Y,Theta = MH_DOC (c1_PDFO, c2_PDFO,c3_PDFO, Xi,Xf)
    ...
    plt.ylabel ('Y [m]')
    plt.legend()
    plt.savefig("POIDS_VARIE_Traj_moy_{}.png".format(data[i]))
...
df.to_csv('RMSE_poids_variable_Trajectoire_moyenne.csv', index = True)

dfi.export(pd.read_csv('RMSE_poids_variable_Trajectoire_moyenne.csv'), 'MH_poids_varie_Trajectoire_moyenne.png')
