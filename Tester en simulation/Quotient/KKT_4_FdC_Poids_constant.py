...
def MH_DOC(c1,c2,c3,c4,Xi,Xf):
    ...
    #opti.callback(lambda i: plt.plot(opti.debug.value(x[1:])- opti.debug.value(x[:n-1]) - taux*(cos(opti.debug.value(theta[:n-1]))*opti.debug.value(v1[:n-1])-sin(opti.debug.value(theta[:n-1]))*opti.debug.value(v2[:n-1])), label = "{}".format(i)))
...
def Katja_Mombaur_IOC (U1,U2,U3 ,V1,V2,W,X,Y,THETA,Xi,Xf) :
    ...
    opti.minimize( R_K(U1,U2,U3,V1,W,V2,X,Y,THETA, Lambda, alpha1, alpha2, alpha3, alpha4 , X[0],Y[0] ,THETA[0] , X[-1] ,Y[-1],THETA[-1] )) 
...
a,b,c,d , e = Katja_Mombaur_IOC (u1,u2,u3 ,v1,v2,w,x,y,o,Xi,Xf) 
